    ...
    for(let i = 0; i < text.length; i++) {
        if(
            ...
            if(text.charAt(i).charCodeAt(0) + shift > 90) {   // Find the difference incase the ascii code after shifts adds up beyond 90, get the difference and add it from the starting ascii code number
                encryptedString += `${String.fromCharCode(64 + text.charAt(i).charCodeAt(0) + shift - 90)}`;  // eg. x('88') -> a('65'): After shift of 3, 88 -> 91, so diffrence is 1, add 64 + 1 -> (65) ASCII Code of 'A' 
            } else {
...
export const getDecryptedText = (text: string, shift: number) => {
    ...
    for(let i = 0; i < text.length; i++) {
        if(
            ...
            if(65 > text.charAt(i).charCodeAt(0) - shift) {   //  Find the difference incase the ascii code after shifts differs below 65, get the difference and subtract it from the last ascii code number of the corresponding letter
